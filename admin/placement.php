...
if(isset($_GET['delid'])){
  ...
  $sql=mysqli_query($conn,"delete from placement where id='$id'");
  if($sql=1){
      header("location:placement.php");
  }
  }

  if(isset($_POST['placementedit1'])){
    $id=$_POST['id'];
    $date = $_POST['date'];
    ...
    $sql="UPDATE `placement` SET `date`='$date',`company_name`='$company_name',`tech`='$tech',`location`='$location'
    ,`placed`='$placed' WHERE id='$id'";
    if (mysqli_query($conn, $sql)){
      // header("location:new_project.php");
      echo "<script>alert('Successfully Updated');</script>";
   } else {
      echo "<script> alert ('connection failed !');window.location.href='placement.php'</script>";
   }
  }
...
<head>
  ...
  <title>Marcks Training | placement</title>
...
<div class="wrapper">
   ...
   <div class="content-wrapper">
            ...
            <div class="col-12">
              ...
              <div class="card">
                <div class="card-header" style="padding:0px;">
                  <h3 class="card-title" style="padding-top:25px; margin-left:10px;">placement</h3>
                  <div class="card-tools my-3" style="text-align:end;">
                    <a class="btn btn-primary" href="placement_form.php" data-tt="tooltip" title=""
                      data-original-title="Click here to Add project" style="margin-right:20px;">Add placement</a>
                  </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
    ...
    ?>
                      <tr>
                       ...
                       <button  type="button" class="btn btn-primary btn-rounded btn-icon placementedit btn-sm" data-toggle="modal" data-id='<?php echo $arr['id']; ?>'
  ...
  <div class="modal-dialog">
    ...
    <button type="button" class="btn-close btn btn-secondary" data-dismiss="modal">Close</button>
      <button type="submit" class="btn btn-primary" name="placementedit1">Save changes</button>
    </div>
  </form>
  </div>
  </div>
 ...
 <script src="plugins/jquery/jquery.min.js"></script>
  ...
  <script>
    ...
    <script>
          ...
          $('.placementedit').click(function(){
            let dnk7 = $(this).data('id');
